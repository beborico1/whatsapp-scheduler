...
def create_scheduled_message(schedule: ScheduledMessageCreate, db: Session = Depends(get_db)):
    ...
    # Schedule the task with ETA (Estimated Time of Arrival) for precise timing
    ...
    print(f"DEBUG: Message {db_schedule.id} scheduled with ETA for {schedule.scheduled_time} (in {time_until_send:.1f} seconds)")
